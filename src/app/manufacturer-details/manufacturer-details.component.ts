...
@Component({
  ...
  <header class="mb-8">
    ...
    <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">
      {{ manufacturerName() }} <span class="text-indigo-600">Collection</span>
    </h1>
  </header>
  ...
  <main class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
    @for (doll of service.dolls(); track doll.id) {
      <app-doll-card [doll]="doll" class="transform hover:-translate-y-1 transition-transform duration-300"></app-doll-card>
    }
  </main>
</div>
  `,
